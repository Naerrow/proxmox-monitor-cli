...
let NOTION_DATA_SOURCE_ID = null;
...
async function getDataSourceId() {
  ...
  if (dataSources.length === 0) {
    throw new Error(
      "Notion database has no data_sources. NOTION_DATABASE_ID를 확인하세요.",
    );
  }

  NOTION_DATA_SOURCE_ID = dataSources[0].id;

  console.log(`Using Notion data source: ${NOTION_DATA_SOURCE_ID}`);
...
async function retrieveDataSource(dataSourceId) {
  ...
  if (!response.ok) {
    const body = await response.text();
    throw new Error(`Retrieve data source failed: ${response.status} ${body}`);
  }

  return response.json();
}

async function queryDataSource(dataSourceId, payload) {
  const response = await fetch(
  ...
  if (!response.ok) {
    const body = await response.text();
    throw new Error(`Query data source failed: ${response.status} ${body}`);
  }
...
async function getDatabaseTitlePropertyName() {
  ...
  if (!properties) {
    ...
    throw new Error("Notion data source properties not found");
  }
  ...
  throw new Error("Notion data source title property not found");
}
...
async function createPageInDataSource(dataSourceId, properties, children) {
  ...
  if (!response.ok) {
    const body = await response.text();
    throw new Error(`Create page failed: ${response.status} ${body}`);
  }
...
async function main() {
  ...
  if (!existingPage) {
    const page = await createPageInDataSource(dataSourceId, properties, [
      ...autoBlocks,
      ...buildManualBlocks(),
    ]);
